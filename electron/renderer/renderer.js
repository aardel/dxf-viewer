...
window.addEventListener('load', async () => {
    try {
        ...
        // Always load value from XML profile at startup
        const profilePath = 'mtl.xml'; // Adjust if needed for active profile
        let xmlProfile = await window.electronAPI.loadXmlProfile(profilePath);
        let showConfigIssues = false;
        if (typeof xmlProfile.ShowConfigIssuesOnStartup !== 'undefined') {
            showConfigIssues = xmlProfile.ShowConfigIssuesOnStartup === true || xmlProfile.ShowConfigIssuesOnStartup === 'true';
        }
        configCheckbox.checked = showConfigIssues;
        configCheckbox.addEventListener('change', async (e) => {
            // Update XML config immediately
            try {
                xmlProfile.ShowConfigIssuesOnStartup = e.target.checked;
                await window.electronAPI.saveXmlProfile(profilePath, xmlProfile);
                showStatus('Startup config saved to profile', 'success');
            } catch (err) {
                showStatus('Failed to save startup config to profile', 'error');
                console.error('Error saving ShowConfigIssuesOnStartup:', err);
            }
        });
        // Show config issues window at startup if enabled in XML
        if (showConfigIssues) {
            setTimeout(async () => {
                await validateConfiguration();
            }, 2000);
