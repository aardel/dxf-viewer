...
// Open Internal Line Types Editor window
ipcMain.handle('open-line-types-manager', async () => {
    const lineTypesWindow = new BrowserWindow({
        ...
        parent: mainWindow,
        modal: false,
        title: 'Internal Line Types Editor v3.0'
    });
    ...
    console.log('🚀 Opening Internal Line Types Editor v3.0 with cache-bust:', timestamp);
    ...
    lineTypesWindow.webContents.once('did-finish-load', () => {
        console.log('🔄 Internal Line Types Editor: Force reloading to clear cache...');
...
// Open Line Type Mapping window
ipcMain.handle('open-line-type-mapping', async () => {
    const mappingWindow = new BrowserWindow({
        ...
        parent: mainWindow,
        modal: false,
        title: 'Line Type Mapping v3.0'
    });

    // Add timestamp to force cache reload and always clear cache
    const timestamp = Date.now();
    const htmlPath = path.join(__dirname, '../renderer/line-type-mapping.html');
    console.log('🚀 Opening Line Type Mapping v3.0 with cache-bust:', timestamp);
    
    mappingWindow.loadFile(htmlPath, { 
        query: { 'cache-bust': timestamp.toString() } 
    });
    
    // Always clear cache to ensure fresh code loads (not just in dev mode)
    mappingWindow.webContents.once('did-finish-load', () => {
        console.log('🔄 Line Type Mapping: Force reloading to clear cache...');
        mappingWindow.webContents.reloadIgnoringCache();
    });
...
ipcMain.handle('process-dxf-file', async (event, { inputPath, outputFolder }) => {
    ...
    console.log('DEBUG: DinGenerator value:', DinGenerator);
        ...
        if (fs.existsSync(activeProfilePath)) {
            ...
            // Load JSON postprocessor config (secondary configuration - overrides XML)
            const jsonConfigPath = path.join(process.cwd(), 'CONFIG', 'postprocessors', `${activeProfileName.replace('.xml', '')}.json`);
            if (fs.existsSync(jsonConfigPath)) {
                const jsonConfig = JSON.parse(fs.readFileSync(jsonConfigPath, 'utf8'));
                // Merge JSON config over XML config
                config = { ...config, ...jsonConfig };
                console.log('DEBUG: Loaded and merged JSON postprocessor configuration');
            } else {
                console.log('DEBUG: No JSON postprocessor config found, using XML profile only');
            }
        } else {
            // Fallback to default config if no active profile
            console.log('DEBUG: No active profile found, using default configuration');
            const defaultConfigPath = path.join(process.cwd(), 'CONFIG', 'postprocessors', 'default_metric.json');
