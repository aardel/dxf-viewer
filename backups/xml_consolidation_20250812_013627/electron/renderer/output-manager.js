...
async function loadStructureConfiguration() {
    try {
        ...
        // Load structure elements from JSON config (for UI persistence)
        const jsonConfig = await window.electronAPI.loadPostprocessorConfig(profileName);
        
        if (jsonConfig && jsonConfig.structure && jsonConfig.structure.header && jsonConfig.structure.footer) {
            headerElements = jsonConfig.structure.header;
            footerElements = jsonConfig.structure.footer;
        ...
        // Load line number settings from XML configuration to UI
        ...
        if (xmlConfig && xmlConfig.units && xmlConfig.units.scalingHeader) {
            ...
            if (scalingElement) {
                ...
                if (xmlConfig.units.scalingHeader.comment) {
                    scalingElement.config.comment = xmlConfig.units.scalingHeader.comment;
                }
            }
        }
...
async function saveStructureConfiguration() {
    try {
        ...
        // Save structure elements to JSON config (for UI persistence)
        const jsonConfig = await window.electronAPI.loadPostprocessorConfig(profileName) || {};
        jsonConfig.structure = {
            header: headerElements,
            footer: footerElements
        };
        await window.electronAPI.savePostprocessorConfig(profileName, jsonConfig);
        
        // Save line number settings to XML configuration (so they're used by DIN generation)
        const enableLineNumbers = document.getElementById('enableLineNumbers')?.checked || false;
        ...
        if (scalingElement && scalingElement.enabled) {
            ...
            config.units.scalingHeader = {
                ...
                comment: scalingElement.config.comment || 'Imperial scaling applied'
            };
        } else {
